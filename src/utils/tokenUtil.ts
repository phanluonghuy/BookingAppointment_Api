...
function verifyEmailToken(email: string, otp: string): string {
  return jwt.sign(
    {
      email,
      otp,
...
function decodeEmailToken(token: string): { email: string; otp: string } | null {
    try {
      const decoded = jwt.verify(
        token,
        process.env.TOKEN_SECRET as string
      ) as { email: string; otp: string }; // Type assertion for the decoded payload
      return decoded;
    } catch (error) {
...
function createPasswordResetToken(_id: string): string {
  ...
  const resetToken = jwt.sign({ _id }, process.env.TOKEN_SECRET as string, {
